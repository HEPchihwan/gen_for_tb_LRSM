...
class MassRecoFromLHE:
    ...
    def _process_event(self, event_lines):
        ...
        # off-shell N-뮤온 짝 찾기
        N_particles = [p for p in particles if p["pid"] == 9900014]
        muons = [p for p in particles if abs(p["pid"]) == 13]

        for Np in N_particles:
            N_parents = {Np["mother1"], Np["mother2"]}
            for mp in muons:
                mu_parents = {mp["mother1"], mp["mother2"]}
                if N_parents & mu_parents:
...
output_csv_path = "/data6/Users/snuintern1/tbchannel/gen_for_tb_LRSM/analysis/result/offshell_count/mass_reco_summary_counts_muon.csv"
output_plot_dir = "/data6/Users/snuintern1/tbchannel/gen_for_tb_LRSM/analysis/result/offshell_count/test/mass_reco_plots_muon"
