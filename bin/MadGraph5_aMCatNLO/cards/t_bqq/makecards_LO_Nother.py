...
while (WRMASS <= WRMASSEND):
    ...
    os.system("cp skeletons_for_LO_Nother/run_card.dat "+DIRNAME+"/"+DIRNAME+"_run_card.dat")     
    os.system("cp skeletons_for_LO_Nother/extramodels.dat "+DIRNAME+"/"+DIRNAME+"_extramodels.dat")
    proclines_path = "skeletons_for_LO_Nother/proc_card.dat"
    custolines_path = "skeletons_for_LO_Nother/customizecards.dat"
    ...
    with open(f"{DIRNAME}/{DIRNAME}_proc_card.dat", "w") as procnew:
        for line in proclines:
            if "###OUTPUT" in line:
                procnew.write(f"width_{DIRNAME} \n")
            else:
                procnew.write(line)
